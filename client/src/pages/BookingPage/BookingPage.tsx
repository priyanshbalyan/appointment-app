...
const BookingPage = (): React.ReactElement => {
	...
	const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>): void => {
		const doctor = doctors?.find(doc => doc.id.toString() === event.target.value) ?? null;
	...
	const handleBook = async (): Promise<void> => {
		...
		try {
      await bookSlots(selectedTimes.map(time => ({ 
				time,
	...
	const renderDays = dateRange.map(day => (
			<button 
				...
				className={selectedDay === day.day ? styles.buttonHighlight : styles.dayButton}>
				{day.week}<br />
				{day.day}
			</button>
		)
	...
	return (
		<div className={styles.container}>
			<FontAwesomeIcon className={styles.icon} icon={faArrowLeft} onClick={handleBackClick} />
			<h1>Booking Appointment</h1>
			<div>
				<select value={selectedDoc?.id} onChange={handleSelectChange}>
					{(doctors ?? []).map(doctor => <option key={doctor.id} value={doctor.id}>{doctor.specialisation}</option>)}
				</select>
				<br />
				<select value={selectedMonth} onChange={handleMonthChange}>
					{monthRange.map(month => <option key={month.month} value={month.value}>{`${month.month} ${month.year}`}</option>)}
				</select> 
			</div>
			...
			/>}
			<div className={styles.divider}></div>
			<button className={styles.bookButton} onClick={handleBook}>Book now {selectedTimes.length > 0 ? `(${selectedTimes.length} selected)` : ''}</button>
		</div>
	);
};
