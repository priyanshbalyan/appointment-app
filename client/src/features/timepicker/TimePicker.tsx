...
const TimePicker = (props: Props): React.ReactElement => {
	...
	const getTimeButtonClass = (timeValue: Date): string => {
		if (includesDate(slots?.map((slot) => slot.time), timeValue))
			return styles.timeDisabled;
		return includesDate(selectedTimes, timeValue)
	...
	if (range.length === 0)
		return (
			<div className={styles.noSlotText}>No open slots available for today</div>
		);
